...
class S2Y:
    ...
    def get_yolo_annotation_info(folder_name, file_name, class_names_array):
        image_path = supervisely_path + "//dataset//"+folder_name+"//img//" + file_name
        print("image_path",image_path)

        image_json_file = supervisely_path + '//dataset//'+folder_name+'//ann//' + file_name + '.json'
        ...
        if len(json_object["objects"])>0:
            ...
            if not skip_copy:
                ...
                #filename, file_ext = os.path.basename(image_path)

                copy_path = yolo_path + '//images//' +folder_name+"_"+os.path.basename(image_path)
    ...
    def create_text_file(folder_name, file_name, class_names_array):
        ...
        class_coord_list = S2Y.get_yolo_annotation_info(folder_name, file_name, class_names_array)
        print("class_coord_list",class_coord_list)

        if len(class_coord_list)>0:
            text_file_path = yolo_path + '//labels//' + os.path.splitext(folder_name+"_"+file_name)[0] + ".txt"
...
def main():
    ...
    if conversion_type == y2s_flag:
        ...
        dataset_folders = [folder for folder in os.listdir(supervisely_path+"//dataset//")]
        for folder in dataset_folders:

            labels_path = supervisely_path + "//dataset//" + folder + "//ann"
            print("labels_path",labels_path)
            for file_path in glob.glob(os.path.join(labels_path, '*.json')):
                with open(file_path) as file:
                    file_name = os.path.basename(file.name)[:-5]
                    S2Y.create_text_file(folder, file_name, class_names_array)
